...
/// Presents `SandboxManagerOverlay` in a borderless child `NSPanel` so that
/// keyboard events are automatically isolated from the main window's terminal.
@MainActor
final class SandboxManagerPanel: NSPanel {
    private static var current: SandboxManagerPanel?
    ...
    private init(contentRect: NSRect, appState: AppState) {
        ...
        let hostingView = NSHostingView(
            rootView: SandboxManagerOverlay()
